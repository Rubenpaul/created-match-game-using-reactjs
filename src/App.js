...
class App extends Component {
  ...
  tick = () => {
    const {isGameCompleted, timer} = this.state
    if (isGameCompleted === false && timer > 0) {
  ...
  render() {
    const {randomNum, ActiveTabId, isGameCompleted, score, timer} = this.state
    const GameImgUrl = imagesList[randomNum].imageUrl
    ...
    return (
      <div className="app-container">
        ...
        {isGameCompleted || timer === 0 ? (
          <ul className="score-card-div">
            <ScoreCard score={score} resetGame={this.resetGame} />
          </ul>
        ) : (
          <>
            <div className="match-image-container">
              <img src={GameImgUrl} alt="match" className="match-image" />
            </div>
